...
class OpenAddressing
  def initialize(size)
   ...
   def []=(key, value)
     puts "key value bracket module (1)"
     print
   ...
   def index(key, size)
     #key.sum % size
     key.to_s.chars.inject(0){|sum, ch| sum + ch.ord} % size
   end
   ...
   def print
     for i in 0..size-1 do
       if !@nodes[i].nil?
         puts "Load factor: #{load_factor} Hash: #{index(@nodes[i].key, self.size)} -> Index: #{i} -> Key: #{@nodes[i].key} -> Value: #{@nodes[i].value}"
       end
     end
   end
 end
